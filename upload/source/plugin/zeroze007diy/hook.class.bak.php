...
class plugin_zeroze007diy {
	...
	function _hidepermit($permitrequire, $message, $pid) {
		global $_G;
	...
	function discuzcode($val){
		...
		if(strpos($msglower, '[/hide]') !== false) {
			if(strpos($msglower, '[hide=p') !== false) {
				$message = preg_replace_callback("/\[hide=p(\d+)\]\s*(.+?)\s*\[\/hide\]/is", create_function('$matches', 'return plugin_zeroze007diy::_hidepermit($matches[1], $matches[2], '.intval($pdateline).');'), $message);
			}
			if(strpos($msglower, '[hide]') !== false) {
				if(in_array($_G['member']['groupid'], $exemptHideGroups) || ($_G['uid'] && in_array($_G['uid'], $exemptHideUids)) /*&& isset($_G['member']['groupexpiry']) && $_G['member']['groupexpiry'] == 0*/){
