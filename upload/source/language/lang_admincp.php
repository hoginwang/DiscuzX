...
',
	...
	'setting_seo_portal_tips' => '<li>站点名称&nbsp;<span class="highlight">{bbname}</span>（应用范围：所有位置）</li><li>当前频道名称&nbsp;<span class="highlight">{curcat}</span>（应用范围：除首页以外）</li><li>一级频道名称&nbsp;<span class="highlight">{firstcat}</span>（应用范围：除首页以外）</li><li>二级频道名称&nbsp;<span class="highlight">{secondcat}</span>（应用范围：除首页以外）</li><li>文章标题&nbsp;<span class="highlight">{subject}</span>（应用范围：文章内容页）</li><li>文章摘要&nbsp;<span class="highlight">{summary}</span>（应用范围：文章内容页）</li><li>文章作者&nbsp;<span class="highlight">{user}</span>（应用范围：文章内容页）</li><li>分页数&nbsp;<span class="highlight">{page}</span>（应用范围：文章列表页、文章内容页）</li>',
	'setting_seo_articlelist' => '文章列表',
	'setting_seo_article' => '文章内容',
	'setting_seo_forum' => '论坛首页',
	'setting_seo_forum_tips' => '<li>站点名称&nbsp;<span class="highlight">{bbname}</span>（应用范围：所有位置）</li><li>当前版块名称&nbsp;<span class="highlight">{forum}</span>（应用范围：除首页以外）</li><li>一级版块名称&nbsp;<span class="highlight">{fup}</span>（应用范围：子版块主题列表页和帖子内容页）</li><li>分区名称&nbsp;<span class="highlight">{fgroup}</span>（应用范围：除首页以外）</li><li>帖子标题&nbsp;<span class="highlight">{subject}</span>（应用范围：帖子内容页）</li><li>主题摘要&nbsp;<span class="highlight">{summary}</span>（应用范围：帖子内容页）</li><li>主题标签&nbsp;<span class="highlight">{tags}</span>（应用范围：帖子内容页）</li><li>分页数&nbsp;<span class="highlight">{page}</span>（应用范围：主题列表页、帖子内容页）</li>',
	'setting_seo_threadlist' => '主题列表',
	'setting_seo_viewthread' => '帖子内容',
	'setting_seo_home' => '家园首页',
	'setting_seo_home_tips' => '<li>站点名称&nbsp;<span class="highlight">{bbname}</span>（应用范围：所有位置）</li><li>日志标题&nbsp;<span class="highlight">{subject}</span>（应用范围：日志内容页）</li><li>日志/相册作者&nbsp;<span class="highlight">{user}</span>（应用范围：日志和相册内容页）</li><li>日志摘要&nbsp;<span class="highlight">{summary}</span>（应用范围：日志内容页）</li><li>日志标签&nbsp;<span class="highlight">{tags}</span>（应用范围：日志内容页）</li><li>相册名称&nbsp;<span class="highlight">{album}</span>（应用范围：相册内容页）</li><li>相册描述&nbsp;<span class="highlight">{depict}</span>（应用范围：相册内容页）</li>',
	'setting_seo_blog' => '日志内容',
	'setting_seo_album' => '相册内容',
	'setting_seo_group' => '群组首页/分类',
	'setting_seo_group_tips' => '<li>站点名称&nbsp;<span class="highlight">{bbname}</span>（应用范围：所有位置）</li><li>当前群组名称&nbsp;<span class="highlight">{forum}</span>（应用范围：除群组首页、分类页）</li><li>一级分类名称&nbsp;<span class="highlight">{first}</span>（应用范围：所有位置）</li><li>二级分类名称&nbsp;<span class="highlight">{second}</span>（应用范围：所有位置）</li><li>当前群组简介&nbsp;<span class="highlight">{gdes}</span>（应用范围：群组和群组帖子页）</li><li>帖子标题&nbsp;<span class="highlight">{subject}</span>（应用范围：群组帖子页）</li><li>主题摘要&nbsp;<span class="highlight">{summary}</span>（应用范围：群组帖子页）</li><li>主题标签&nbsp;<span class="highlight">{tags}</span>（应用范围：群组帖子内容页）</li><li>分页数&nbsp;<span class="highlight">{page}</span>（应用范围：主题列表页、帖子内容页）</li>',
...
<br />(3)SPH_RANK_NONE，禁用评分的模式，这是最快的模式。实际上这种模式与布尔搜索相同。所有的匹配项都被赋予权重1',
	...
	'setting_antitheft_tips' => '<li>防采集功能不影响正常用户浏览检验。</li><li>可防止本站主题、文章和日志被非法采集。</li><li><span style="color:red">当IP同时存在于黑/白名单时，白名单优先。</span></li><li>开启防采集功能后帖子列表页面的帖子预览功能将不可使用。</li>',
	...
	'templates_maint_nondefault_comment' => '<li>本套系中并不要求必须完整，所缺少的模板将使用默认模板代替。</li><li>如果模板名称显示<span color="#FF0000">红色</span>，表示此模板与默认模板不相同；显示<span color="#00FF00">绿色</span>，表示此模板为用户添加的模板文件，默认模板没有相应的模板文件存在。</li><li>如果您要发布此套模板，建议您把与默认模板相同的模板文件删除后再发布。</li>',
	...
	'members_edit_uinblack_notice' => '<br /><span color="red">警告: 当前用户未设置独立密码，被封禁后将无法登录本站</span>',
...
<li><b>由于涉及现金交易，为避免因操作不当而造成的资金损失，请在开始使用财付通积分交易功能(不包含财付通按钮功能)前，务必仔细阅读《用户使用说明书》中有关电子商务的部分，当确认完全理解和接受相关流程及使用方法后再进行相关设置。</b></li><li>您可以设置允许用户通过现金在线支付的方式，为其交易积分账户充值，用于购买帖子内容、购买用户组权限、积分转账或用户组升级等功能。财付通积分交易功能，需在“积分设置”中启用交易积分，并同时设置相应的积分策略以满足不同场合的需要。请务必正确设置您的收款财付通账号，否则将造成用户付款后积分无法实时到账，造成大量需要人工处理的订单信息。</li><li>除 Discuz! 官方网站或官方论坛另行通知以外，Discuz! 提供的财付通支付服务每笔交易收取 1% 的手续费。请及时关注相关业务的最新通知，各项政策或流程的变更、调整，以 Discuz! 官方网站或官方论坛提供的信息为准。</li><li>您使用财付通服务是建立在完全自愿的基础上，除 Discuz! 因主观恶意的因素造成的资金损失以外，腾讯云不对因使用此功能造成的任何损失承担责任。</li><li>财付通服务热线：0755-83762288-2(全天24小时服务热线)</li>',
	...
	'filecheck_tips' => '<li>当论坛出现异常或者被非法入侵后，您应当使用本功能校验论坛程序文件，以确保论坛安全运行</li><li>“<span color="red">丢失</span>”的文件您应当通过 FTP 及时将文件补充进来，否则可能论坛运行不正常</li><li>“<span color="red">被修改</span>”、“<span color="red">未知</span>”的文件应当引起您的警惕，必须确认文件是您自己修改或者是某个插件添加的，否则请删除未知文件和恢复被修改的文件</li>',
	...
	'hookcheck_tips' => '<li>“<span color="red">嵌入点</span>”是模板文件中的“&lt;!--{hook/xxx}--&gt;”代码,模板文件丢失嵌入点代码会造成云服务（如QQ群、腾讯分析等）等插件无法正常使用</li><li>缺失“<span color="red">嵌入点</span>”的模板文件需要使用对比工具,对比原始文件和服务器上对应的模板文件进行对比,将丢失的嵌入点代码补充好</li>',
	...
	'db_zip' => 'ZIP',
	'db_volume' => '卷数',
	'db_export_tips_nouc' => '<li><span color="blue"><b>您当前的数据备份不包含 UCenter，会影响到您的会员数据，请点击<a href="{uc_backup_url}">这里</a>单独备份 UCenter 数据</b></span></li><li>数据备份功能根据您的选择备份全部Discuz!数据，导出的数据文件可用“数据恢复”功能或 phpMyAdmin 导入。</li>',
	...
	'upgrade_tips' => '<li><span color="red">在线升级前，请确认您已经备份程序及数据库</span></li>',
	...
	'portalcategory_listprimaltplname_comment' => '将模板文件上传到模板目录的portal目录下，如：template/default/portal目录下，文件名必须为<span color="red">list</span>_*.htm，*为自定义文件名<br />如果要重新选择模板，请确保新模板与原模板中可拖拽区域具有相同的 ID，否则将会丢失部分或全部原 DIY 数据',
	'portalcategory_viewprimaltplname' => '文章页模板名',
	'portalcategory_viewprimaltplname_comment' => '将模板文件上传到模板目录的portal目录下，如：template/default/portal目录下，文件名必须为<span color="blue">view</span>_*.htm，*为自定义文件名<br />如果要重新选择模板，请确保新模板与原模板中可拖拽区域具有相同的 ID，否则将会丢失部分或全部原 DIY 数据',
