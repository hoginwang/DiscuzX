 ...
 *
 *      $Id: misc_webupload.php 25756 2011-11-22 02:47:45Z zhangguosheng $
 */
...
if($op == "finish") {
	...
	$uploadurl = urlencode(getsiteurl().'home.php?mod=misc&ac=webupload'.($iscamera ? '&op=screen' : ($isdoodle ? '&op=doodle&from=':'')));
	if($isupload) {
	...
	$feedurl = urlencode(getsiteurl().'home.php?mod=misc&ac=webupload&op=finish&random='.random(8).'&albumid=');
...
include template("home/misc_webupload");
