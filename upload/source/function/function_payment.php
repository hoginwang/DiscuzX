 ...
 *
 *      $Id: function_payment.php 36342 2021-05-17 15:20:43Z dplugin $
 */
...
function payment_channels() {
	...
	$result['weixin'] = array(
		'id' => 'weixin',
		'title' => lang('spacecp', 'payment_weixin'),
		'logo' => 'static/image/common/weixin.png',
		'enable' => 0
	);

	$channels = C::t('common_setting')->fetch_all_setting(array('ec_weixin', 'ec_alipay'), true);
	if ($channels['ec_alipay']['on']) {
		$result['alipay']['enable'] = 1;
	}
	if ($channels['ec_weixin']['on']) {
		$result['weixin']['enable'] = 1;
	}
	return $result;
}

function payment_newinstance($channel) {
...
function payment_create_order($type, $subject, $description, $amount, $return_url, $params = NULL, $fee = 0, $expire = 3600) {
	global $_G;
...
function payment_finish_order($channel, $out_biz_no, $trade_no, $payment_time) {
	$lock_order_key = 'payment_notify_' . $out_biz_no;
	if (!discuz_process::islocked($lock_order_key, 600)) {
		$order = C::t('common_payment_order')->fetch_by_biz_no($out_biz_no);
		if (!$order || $order['status']) {
			discuz_process::unlock($lock_order_key);
			return true;
		}
		...
		C::t('common_payment_order')->update($order['id'], array(
		...
		if (file_exists($callback_class)) {
			...
			if (class_exists($class_name)) {
				try {
					...
					writelog('errorpayment', '[ERROR] callback exception, ' . ', message: ' . $e->getMessage());
				}
			}
		}
		DB::query('commit');
...
function payment_query_order($channel, $order_id) {
	...
	if ($result['code'] == 200 && $order['status'] != 1 && $result['data']) {
		payment_finish_order($channel, $order['out_biz_no'], $result['data']['trade_no'], $result['data']['payment_time']);
	}
	return $result;
}

function payment_refund($refund_no, $order_id, $amount, $refund_desc) {
	global $_G;
	$order = C::t('common_payment_order')->fetch($order_id);
	...
	$payment = payment_newinstance($order['channel']);
...
function payment_refund_status($refund_no, $order_id) {
	$order = C::t('common_payment_order')->fetch($order_id);
	if (!$order || $order['status'] != 1) {
		return array('code' => 500, 'message' => lang('message', 'payment_order_no_exist'));
	}
...
function payment_transfer($channel, $transfer_no, $amount, $uid, $realname, $account, $title = '', $desc = '') {
	global $_G;
...
function payment_transfer_status($transfer_no) {
	...
	} else if ($refund_order['status'] == 2) {
		return array('code' => 200, 'data' => array('transfer_time' => $refund_order['trade_time']));
	}

	$payment = payment_newinstance($refund_order['channel']);
