        ...
        if ($output['preg']) {
            foreach ($data['footernavs'] as $id => $nav) {
                foreach ($output['preg']['search'] as $key => $value) {
                    $data['footernavs'][$id]['code'] = preg_replace_callback($value, function ($matches) use ($output, $key) {
                        return $output['preg']['replace'][$key];
                    }, $nav['code']);
                }
            }
            foreach ($data['spacenavs'] as $id => $nav) {
                foreach ($output['preg']['search'] as $key => $value) {
                    $data['spacenavs'][$id]['code'] = preg_replace_callback($value, function ($matches) use ($output, $key) {
                        return $output['preg']['replace'][$key];
                    }, $nav['code']);
                }
            }
            foreach ($data['mynavs'] as $id => $nav) {
                foreach ($output['preg']['search'] as $key => $value) {
                    $data['mynavs'][$id]['code'] = preg_replace_callback($value, function ($matches) use ($output, $key) {
                        return $output['preg']['replace'][$key];
                    }, $nav['code']);
                }
            }
            foreach ($data['topnavs'] as $id => $nav) {
                foreach ($output['preg']['search'] as $key => $value) {
                    $data['topnavs'][$id]['code'] = preg_replace_callback($value, function ($matches) use ($output, $key) {
                        return $output['preg']['replace'][$key];
                    }, $nav['code']);
                }
            }
            foreach ($data['plugins']['jsmenu'] as $id => $nav) {
                foreach ($output['preg']['search'] as $key => $value) {
                    $data['plugins']['jsmenu'][$id]['url'] = preg_replace_callback($value, function ($matches) use ($output, $key) {
                        return $output['preg']['replace'][$key];
                    }, $nav['url']);
                }
            }
