...
if($('$showid')) {
	...
	var string = '<input name="secqaahash" type="hidden" value="$idhash" /><input name="secqaakeycode" id="secqaakeycode_$idhash" type="hidden" value="" />' + sectplcode[0] + '$seclang[secqaa]' + sectplcode[1] + '<input name="secanswer" id="secqaaverify_$idhash" type="text" autocomplete="off" style="{$imemode}width:100px" class="txt px vm" onblur="checksec(\'qaa\', \'$idhash\')"" onkeydown="if(event.keyCode == 13){\$(\'secqaakeycode_$idhash\').value = 13}else{\$(\'secqaakeycode_$idhash\').value = 0}" />' +
