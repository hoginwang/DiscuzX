...
if($('$showid')) {
	if(!$('v$showid')) {
		...
		var string = '<input name="seccodehash" type="hidden" value="$idhash" /><input name="seccodemodid" type="hidden" value="$modid" /><input name="seccodekeycode" id="seccodekeycode_$idhash" type="hidden" value="" />' + sectplcode[0] + '$seclang[seccode]' + sectplcode[1] + '<input name="seccodeverify" id="seccodeverify_$idhash" type="text" autocomplete="off" style="{$imemode}width:100px" class="txt px vm" onblur="checksec(\'code\', \'$idhash\', 0, null, \'$modid\')"" onkeydown="if(event.keyCode == 13){\$(\'seccodekeycode_$idhash\').value = 13}else{\$(\'seccodekeycode_$idhash\').value = 0}" />' +
