...
if($operation == 'alipay') {
	...
	if(!submitcheck('alipaysubmit')) {
		...
		$data = array('pay_alipay_status' => intval($settingsnew['pay_alipay_status']),
			'pay_alipay_appid' => trim($settingsnew['pay_alipay_appid']),
			'pay_alipay_seller_id' => trim($settingsnew['pay_alipay_seller_id']),
			'pay_alipay_publickey' => trim($settingsnew['pay_alipay_publickey']),
			'pay_alipay_merchantprivatekey' => trim($settingsnew['pay_alipay_merchantprivatekey']));
	...
	if($_GET['cert']=='update'){
		...
		if($pay_wechat->getCertificates()){
			cpmsg('pay_wechat_update_succeed', 'action=pay&operation=wechat', 'succeed');
		}else{
			cpmsg('pay_wechat_update_fail', 'action=pay&operation=wechat', 'error');
		}
	}
	...
	if($_GET['cert']=='privateKey'){
		...
		if(file_exists($file)){
			$pay_wechat = new pay_wechat;
			$pay_wechat_privateKey = $pay_wechat->updatePrivateKey($file);
			$data = array('pay_wechat_privateKey' => $pay_wechat_privateKey);
			C::t('common_setting')->update_batch($data);
			updatecache('setting');
			@unlink($file);
			cpmsg('pay_wechat_privateKey_succeed', 'action=pay&operation=wechat', 'succeed');
		}else{
			cpmsg('pay_wechat_privateKey_fail', 'action=pay&operation=wechat', 'error');
		}
	}
	...
	if(!submitcheck('wechatsubmit')) {
		...
		$data = array('pay_wechat_status' => intval($settingsnew['pay_wechat_status']),
			...
			'pay_wechat_mchid' => trim($settingsnew['pay_wechat_mchid']),
			'pay_wechat_key' => trim($settingsnew['pay_wechat_key']),
		...
		if($data['pay_wechat_cert']!=$settings['pay_wechat_cert']){
			$pay_wechat->mkdirs(DISCUZ_ROOT.$data['pay_wechat_cert']);
		}
		updatecache('setting');
