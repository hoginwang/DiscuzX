...
if($operation == 'alipay') {
	...
	if(!empty($checktype)) {
		if($checktype == 'credit') {
			...
			$pay_url = payment_create_order('payment_credit', $lang['ec_alipay_checklink_credit'], $lang['ec_alipay_checklink_credit'], 1, $return_url);
			ob_end_clean();
	...
	if(!submitcheck('alipaysubmit')) {
		...
		showsubmenu('nav_ec', array(
			...
			array('nav_ec_weixin', 'ec&operation=weixin', 0),
	...
	$weixinsettings = C::t('common_setting')->fetch_setting('ec_weixin', true);
	if(!empty($checktype)) {
		if($checktype == 'credit') {
			require_once libfile('function/payment');
			$return_url = $_G['siteurl'] . 'home.php?mod=spacecp&ac=credit';
	...
	if(!submitcheck('weixinsubmit')) {
		...
		showsubmenu('nav_ec', array(
			array('nav_ec_config', 'setting&operation=ec', 0),
			array('nav_ec_tenpay', 'ec&operation=tenpay', 0),
			array('nav_ec_weixin', 'ec&operation=weixin', 1),
		...
		/*search={"nav_ec":"action=setting&operation=ec","nav_ec_weixin":"action=ec&operation=weixin"}*/
		showtips('ec_weixin_tips');
		showformheader('ec&operation=weixin');

		showtableheader('','nobottom');
		showtitle('ec_weixin');
		showtagheader('tbody', 'alipay_weixin', true);
		showsetting('ec_weixin_on', 'settingsnew[on]', $weixinsettings['on'], 'radio');
		require_once libfile('function/payment');
		$wxpayment = payment_newinstance('weixin');
		$check = array();
		$weixinsettings['ec_weixin_version'] ? $check['true'] = "checked" : $check['false'] = "checked";
		...
		showsetting('ec_weixin_appsecret', 'settingsnew[appsecret]', $weixin_securitycodemask, 'text');
		showsetting('ec_weixin_mch_id', 'settingsnew[mch_id]', $weixinsettings['mch_id'], 'text');
		showtagfooter('tbody');

		showtagheader('tbody', 'api_version_2', !$weixinsettings['ec_weixin_version']);
		$weixin_securitycodemask = $weixinsettings['v1_key'] ? $weixinsettings['v1_key'][0].'********'.substr($weixinsettings['v1_key'], -4) : '';
		showsetting('ec_weixin_v1_key', 'settingsnew[v1_key]', $weixin_securitycodemask, 'text');
		showsetting('ec_weixin_v1_cert', 'settingsnew[v1_cert_path]', $weixinsettings['v1_cert_path'], 'text', '', 0, lang('admincp', 'ec_weixin_v1_cert_comment', array('randomstr' => random(10))));
		showtagfooter('tbody');

		showtagheader('tbody', 'api_version_3', $weixinsettings['ec_weixin_version']);
		$weixin_securitycodemask = $weixinsettings['v3_key'] ? $weixinsettings['v3_key'][0].'********'.substr($weixinsettings['v3_key'], -4) : '';
		showsetting('ec_weixin_v3_key', 'settingsnew[v3_key]', $weixin_securitycodemask, 'text');
		$weixin_securitycodemask = $weixinsettings['v3_private_key'] ? substr($weixinsettings['v3_private_key'], 0, 40).'********'.substr($weixinsettings['v3_private_key'], -40) : '';
		showsetting('ec_weixin_v3_private_key', 'settingsnew[v3_private_key]', $weixin_securitycodemask, 'textarea');
		$weixin_securitycodemask = $weixinsettings['v3_serial_no'] ? $weixinsettings['v3_serial_no'][0].'********'.substr($weixinsettings['v3_serial_no'], -4) : '';
		showsetting('ec_weixin_v3_serial_no', 'settingsnew[v3_serial_no]', $weixin_securitycodemask, 'text');
		showtagfooter('tbody');

		showsetting('ec_weixin_check', '', '',
			'<a href="'.ADMINSCRIPT.'?action=ec&operation=weixin&checktype=credit" target="_blank">'.$lang['ec_weixin_checklink_credit'].'</a><br />'
		);
		/*search*/
		showtableheader('', 'notop');
		showsubmit('weixinsubmit');
		showtablefooter();
		showformfooter();
		...
		$settingsnew = $_GET['settingsnew'];
		foreach ($settingsnew as $name => $value) {
			if($value == $weixinsettings[$name] || strpos($value, '********') !== false) continue;
			$value = daddslashes($value);
			$weixinsettings[$name] = $value;
		}
		C::t('common_setting')->update_setting('ec_weixin', $weixinsettings);
		updatecache('setting');

		if($weixinsettings['ec_weixin_version'] && $weixinsettings['appid'] && $weixinsettings['mch_id'] && $weixinsettings['v3_key'] && $weixinsettings['v3_private_key'] && $weixinsettings['v3_serial_no']) {
			require_once libfile('function/payment');
			$payment = payment_newinstance('weixin');
			$result = $payment -> v3_weixin_certificates();
			if($result['code'] == 200) {
				$weixinsettings['v3_certificates'] = $result['data'];
			}
			C::t('common_setting')->update_setting('ec_weixin', $weixinsettings);
			updatecache('setting');
		}

		cpmsg('weixin_succeed', 'action=ec&operation=weixin', 'succeed');
	}
	...
	if(submitcheck('querysubmit')) {
		...
		require_once libfile('function/payment');
		$result = payment_query_order($channel, $order_id);
		if($result['code'] == 200) {
		...
		foreach ($channels as $channel) {
			$channelradios .= '<li><input class="radio" type="radio" name="channel" value="'.$channel['id'].'">&nbsp;'.$channel['title'].'</li>';
		}
		$channelradios .= '</ul>';
	...
	if($_GET['op'] == 'query') {
		...
		require_once libfile('function/payment');
		$result = payment_transfer_status($transfer_no);
		if($result['code'] == 200) {
		...
		require_once libfile('function/payment');
		$result = payment_transfer($order['channel'], $order['out_biz_no'], $order['amount'], $order['uid'], $order['realname'], $order['account'], $order['subject'], $order['description']);
		if($result['code'] == 200) {
