...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	foreach($threadlist as $thread) {
		...
		if($thread['authorid'] && $thread['author']) {
			$thread['author'] = "<a href=\"?action=members&operation=search&uid=$thread[authorid]&submit=yes&formhash".constant("FORMHASH")."\" target=\"_blank\">$thread[author]</a>";
		} elseif($thread['authorid'] && !$thread['author']) {
			$thread['author'] = "<a href=\"?action=members&operation=search&uid=$thread[authorid]&submit=yes&formhash".constant("FORMHASH")."\" target=\"_blank\">$lang[anonymous]</a>";
		} else {
