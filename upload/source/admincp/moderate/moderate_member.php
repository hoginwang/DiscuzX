...
EOT;
	...
	if(!submitcheck('prunesubmit', 1)) {
		...
		if((!$membernum = count($uids))) {
			...
			cpmsg('members_delete_confirm', "action=moderate&operation=members&do=del&submitmore=".rawurlencode($_GET['submitmore'])."&regbefore=".rawurlencode($_GET['regbefore'])."&regip=".rawurlencode($_GET['regip'])."&prunesubmit=yes&formhash".constant("FORMHASH"), 'form', array('membernum' => $membernum));
		} else {
