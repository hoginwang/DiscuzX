...
$env_ok = true;
$now_ver = array("PHP" => constant("PHP_VERSION"), "MySQL" => helper_dbtool::dbversion(), "Extensions" => array("filter_var" => function_exists("filter_var"), "GD" => (function_exists('gd_info') ? preg_replace('/[^0-9.]+/','',gd_info()['GD Version']) : false)));
$req_ver = array("PHP" => '5.3', "MySQL" => '5.0', "Extensions" => array("filter_var" => true, "GD" => '1.0'));
$sug_ver = array("PHP" => '7.1', "MySQL" => '5.7', "Extensions" => array("filter_var" => true, "GD" => '2.0'));
foreach ($now_ver as $key => $value) {
	if (version_compare($value, $req_ver[$key], '<=')) {
...
foreach ($now_ver["Extensions"] as $key => $value) {
	if($req_ver["Extensions"][$key] === true) {
		if (!$value) {
			showtablerow('', array('', 'class="td21" style="text-align:right;"'),
	...
	} else if (version_compare($value, $req_ver["Extensions"][$key], '<=')) {
		showtablerow('', array('', 'class="td21" style="text-align:right;"'),
			'<em class="unfixed">'.lang("admincp", "req_ver_too_low", array('req' => $key, 'now_ver' => $value, 'sug_ver' => $sug_ver["Extensions"][$key], 'req_ver' => $req_ver["Extensions"][$key])).'</em>'
		);
		$env_ok = false;
	}
}
