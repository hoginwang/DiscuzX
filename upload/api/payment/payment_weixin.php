 ...
 *
 *      $Id: payment_weixin.php 36342 2021-05-17 14:15:31Z dplugin $
 */
...
class payment_weixin extends payment {

	public function __construct() {
		global $_G;
		$this->settings = C::t('common_setting')->fetch_setting('ec_weixin', true);
		$this->notify_url = $_G['siteurl'] . 'api/payment/notify/notify_weixin.php';
		parent::__construct();
	}
	...
	public function pay($order) {
		...
		$device = $this->weixin_device();
		if ($device) {
			if (in_array($device, array('weixin'))) {
				$redirect_uri = $_G['siteurl'] . 'home.php?mod=spacecp&ac=payment&op=pay&sop=wxjsapi&&order_id=' . $order['id'];
				$redirect_uri = urlencode($redirect_uri);
				$state = md5(random(32));
				$pay_url = $this->weixin_authorize($redirect_uri, $state);
				return array('code' => 200, 'url' => $pay_url);
			} else {
				if ($this->settings['ec_weixin_version']) {
					return $this->v3_weixin_h5_pay($order);
				} else {
					return $this->weixin_unifiedorder_pay($order, 'MWEB');
				}
			}
		} else {
			if ($this->settings['ec_weixin_version']) {
				return $this->v3_weixin_native_pay($order);
			} else {
				return $this->weixin_unifiedorder_pay($order, 'NATIVE');
			}
		}
	}
	...
	public function status($out_biz_no) {
		...
		if ($this->settings['ec_weixin_version']) {
			return $this->v3_weixin_query_order($out_biz_no);
		} else {
			return $this->weixin_order_query($out_biz_no);
		}
	}
	...
	public function refund($refund_no, $trade_no, $total_amount, $refund_amount, $refund_desc) {
		...
		if ($this->settings['ec_weixin_version']) {
			return $this->v3_weixin_refund($refund_no, $trade_no, $total_amount, $refund_amount, $refund_desc);
		} else {
			return $this->weixin_refund($refund_no, $trade_no, $total_amount, $refund_amount, $refund_desc);
		}
	}
	...
	public function refund_status($refund_no, $trade_no) {
		...
		if ($this->settings['ec_weixin_version']) {
			return $this->v3_weixin_refund_query($refund_no);
		} else {
			return $this->weixin_refund_status($refund_no);
		}
	}
	...
	public function pay_jsapi($order, $openid) {
		...
		if ($this->settings['ec_weixin_version']) {
			return $this->v3_weixin_h5_jsapi($order, $openid);
		} else {
			return $this->weixin_unifiedorder_pay($order, 'JSAPI', $openid);
		}
	}

	public function weixin_jsapidata($prepay_id) {
		$jsapidata = ['appId' => $this->settings['appid'], 'timeStamp' => time() . '', 'nonceStr' => $this->weixin_nonce(), 'package' => 'prepay_id=' . $prepay_id, 'signType' => 'MD5',];
		$jsapidata['paySign'] = $this->weixin_sign($jsapidata);
		return json_encode($jsapidata);
	}

	public function weixin_access_token_by_code($code) {
		...
		return $this->weixin_request($api);
	}

	public function weixin_sign_verify() {
		$xml = file_get_contents('php://input');
		writelog('errorpayment', '[INFO] xml: ' . $xml);
		$data = $this->weixin_x2o($xml);
		$sign = $this->weixin_sign($this->settings['v1_key'], $data, 1);
	...
	public function v3_weixin_sign_verify() {
		...
		return $this->v3_weixin_decrypt2string($resource['associated_data'], $resource['nonce'], $resource['ciphertext']);
	}

	public function v3_weixin_support() {
	...
	public function v3_weixin_certificates() {
		global $_G;
		$api = SDK_WEIXIN_PAY_V3_CERTIFICATES;
		$res = $this->v3_weixin_request_json($api, '', 'GET');
		...
		if ($res['data']) {
			foreach ($res['data'] as $item) {
				...
				$data = $this->v3_weixin_decrypt2string($item['associated_data'], $item['nonce'], $item['ciphertext']);
				$list[$serial_no] = $data;
			}
		}
	...
	private function weixin_unifiedorder_pay($order, $type = 'NATIVE', $openid = null) {
		global $_G;
		...
		$data = array('appid' => $this->settings['appid'], 'mch_id' => $this->settings['mch_id'], 'nonce_str' => $this->weixin_nonce(), 'sign_type' => 'MD5', 'body' => $order['subject'], 'detail' => $order['description'], 'out_trade_no' => $order['out_biz_no'], 'total_fee' => intval($order['amount']), 'spbill_create_ip' => $_G['clientip'], 'time_expire' => dgmdate(time() + 86400, 'YmdHis'), 'notify_url' => $this->notify_url, 'trade_type' => $type,);
		if ($openid) {
			$data['openid'] = $openid;
		}
		$data['sign'] = $this->weixin_sign($this->settings['v1_key'], $data);
		$data = $this->weixin_o2x($data);

		$api = SDK_WEIXIN_PAY_UNIFIEDORDER;
		$res = $this->weixin_request_xml($api, $data);
		if (strtoupper($_G['charset'] != 'UTF-8')) {
			$res = diconv($res, 'UTF-8', $_G['charset']);
		}
		$res = $this->weixin_x2o($res);
	...
	private function weixin_order_query($out_biz_no) {
		global $_G;
		$data = ['appid' => $this->settings['appid'], 'mch_id' => $this->settings['mch_id'], 'out_trade_no' => $out_biz_no, 'nonce_str' => $this->weixin_nonce(), 'sign_type' => 'MD5'];
		$data['sign'] = $this->weixin_sign($this->settings['v1_key'], $data);
		$data = $this->weixin_o2x($data);
	...
	private function weixin_authorize($redirect_uri, $state, $scope = 'snsapi_base') {
		$appid = $this->settings['appid'];
		$redirect_uri = urlencode($redirect_uri);
	...
	private function weixin_refund($refund_no, $trade_no, $total_amount, $refund_amount, $refund_desc) {
		global $_G;
		if (strtoupper($_G['charset'] != 'UTF-8')) {
			$refund_desc = diconv($refund_desc, $_G['charset'], 'UTF-8');
		}
		$data = ['appid' => $this->settings['appid'], 'mch_id' => $this->settings['mch_id'], 'nonce_str' => $this->weixin_nonce(), 'sign_type' => 'MD5', 'transaction_id' => $trade_no, 'out_refund_no' => $refund_no, 'total_fee' => $total_amount, 'refund_fee' => $refund_amount, 'refund_desc' => $refund_desc];
		$data['sign'] = $this->weixin_sign($this->settings['v1_key'], $data);
		$data = $this->weixin_o2x($data);
		$api = SDK_WEIXIN_PAY_REFUND;
		$res = $this->weixin_request_xml($api, $data, true);
	...
	private function weixin_refund_status($refund_no) {
		global $_G;
		$data = ['appid' => $this->settings['appid'], 'mch_id' => $this->settings['mch_id'], 'nonce_str' => $this->weixin_nonce(), 'sign_type' => 'MD5', 'out_refund_no' => $refund_no,];
		$data['sign'] = $this->weixin_sign($this->settings['v1_key'], $data);
		$data = $this->weixin_o2x($data);
	...
	private function v3_weixin_native_pay($order) {
		global $_G;
		if (strtoupper($_G['charset'] != 'UTF-8')) {
			$order['subject'] = diconv($order['subject'], $_G['charset'], 'UTF-8');
			$order['description'] = diconv($order['description'], $_G['charset'], 'UTF-8');
		...
		$res = $this->v3_weixin_request_json($api, json_encode($data));
		$res = json_decode($res, true);
		if ($res['code_url']) {
	...
	private function v3_weixin_h5_pay($order) {
		global $_G;
		if (strtoupper($_G['charset'] != 'UTF-8')) {
			$order['subject'] = diconv($order['subject'], $_G['charset'], 'UTF-8');
			$order['description'] = diconv($order['description'], $_G['charset'], 'UTF-8');
	...
	private function v3_weixin_h5_jsapi($order, $openid) {
		global $_G;
		if (strtoupper($_G['charset'] != 'UTF-8')) {
			$order['subject'] = diconv($order['subject'], $_G['charset'], 'UTF-8');
			$order['description'] = diconv($order['description'], $_G['charset'], 'UTF-8');
	...
	private function v3_weixin_query_order($out_biz_no) {
		global $_G;
		$api = SDK_WEIXIN_PAY_V3_TRANSACTIONS_OUTTRADENO;
		$res = $this->v3_weixin_request_json($api . $out_biz_no . '?mchid=' . $this->settings['mch_id'], '', 'GET');
		$res = json_decode($res, true);
	...
	private function v3_weixin_refund($refund_no, $trade_no, $total_amount, $refund_amount, $refund_desc) {
		global $_G;
		if (strtoupper($_G['charset'] != 'UTF-8')) {
			$refund_desc = diconv($refund_desc, $_G['charset'], 'UTF-8');
		}
	...
	private function v3_weixin_refund_query($refund_no) {
		global $_G;
		$api = SDK_WEIXIN_PAY_V3_REFUND_DOMESTIC_REFUNDS_QUERY;
		$res = $this->v3_weixin_request_json($api . $refund_no, '', 'GET');
		$res = json_decode($res, true);
		if ($res['status'] == 'SUCCESS') {
			return array('code' => 200, 'data' => array('refund_time' => strtotime($res['success_time'])));
		} else if ($res['status']) {
	...
	private function weixin_o2x($data) {
		$xml = '<xml>';
	...
	private function weixin_x2o($xml) {
		libxml_disable_entity_loader(true);
		$data = json_decode(json_encode(simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
		return $data;
	}

	private function weixin_sign($token, $data, $sign = 0) {
		ksort($data);
		$signstr = '';
		foreach ($data as $key => $value) {
	...
	private function v3_weixin_authorization($api, $method, $json) {
		$url_values = parse_url($api);
		$timestamp = time();
		$nonce = $this->weixin_nonce();
	...
	private function v3_weixin_decrypt2string($associateddata, $noncestr, $ciphertext) {
	...
	private function weixin_request($api, $data = null) {
		$ch = curl_init();
	...
	private function weixin_request_xml($api, $xml, $cert = false) {
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $api);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
		...
		if ($cert) {
			...
			curl_setopt($ch, CURLOPT_CAINFO, DISCUZ_ROOT . $this->settings['v1_cert_path']);
		}
		curl_setopt($ch, CURLOPT_HEADER, 1);
		curl_setopt($ch, CURLOPT_POST, 1);
	...
	private function v3_weixin_request_json($api, $json = '', $method = 'POST') {
		$headers = array('Accept: application/json', 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36', 'content-type: application/json', 'Authorization: WECHATPAY2-SHA256-RSA2048 ' . $this->v3_weixin_authorization($api, $method, $json));
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $api);
	...
	private function weixin_device() {
