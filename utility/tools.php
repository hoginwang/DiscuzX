 ...
 *
 *      $Id: tools.php 2021-04-07 20:00:43Z Criss updated $
 */
...
END;
    ...
    case 'editpass':
        if($_POST['act'] == 'delepluges'){
            ...
            $pludata = mysqli_fetch_array(mysqli_query($t->db,$sql), MYSQL_ASSOC);
...
END;
    ...
    if($filelist){
        ...
        print<<<END
            <span style="color:red;font-weight: bold; font-size: x-large;">友情提醒：自动重装功能已经失效，建议前往<a href="https://gitee.com/Discuz/DiscuzX/releases" target="_blank">Gitee/DiscuzX</a>进行下载更新！</span>
END;
		echo "<h3 style=\"color: green; font-weight: bolder; margin: 6px;  padding: 0;\">全新安装: ".TOOLS_DISCUZ_VERSION."</h3>";
		print<<<END

		<form action="?action={$action}" method="post">
    		<table id="reinstallall">
    			<tr><td>{$data}
                <span style="color:red"> * 全新安装过程默认会将所有文件备份一次, 请等待完成!</span>
                </td></tr>
    		</table>
			<input type="submit" onclick="this.style.display='none'" name="reinstallallsubmit" value="提 &nbsp; 交">
		</form>
        <br />
        <h3 style="color: green; font-weight: bolder; margin: 6px;  padding: 0;">二次安装</h3>
        <form action="?action={$action}" method="post">
    		<table id="reinstall_sec">
			...
			<input type="submit" onclick="this.style.display='none'" name="reinstallsecsubmit" value="提 &nbsp; 交">
		</form>
END;
...
show_footer();
	...
	foreach($founders as $userid) {
		...
		$foundernames[] = mysqli_fetch_row(mysqli_query($t->db,$sql))[0]."[{$userid}]";
	}

	$foundernames = implode($foundernames, ',');
	$sql = "SELECT username,uid FROM ".DB_PRE."common_member WHERE `adminid`='1' ORDER BY `uid` ASC";
	$query = mysqli_query($t->db,$sql) or dir(mysqli_error($this->db));
	$adminnames = array();
	while($row = mysqli_fetch_row($query))
    {
		$adminnames[] = $row[0]."[{$row[1]}]";
	}
	...
	if(!empty($_POST['setadminsubmit'])) {
		...
		if($_GET['password'] != NULL) {
			...
			$salt = mysqli_fetch_row(mysqli_query($t->db,$sql))[0];
	...
	} else if(!empty($_POST['clrtimelmtsubmit'])) {
        $sql = "DELETE FROM `pre_common_failedip`";
        $res = mysqli_query($t->db,$sql);
        $rows1= mysqli_affected_rows($t->db);

        $sql = "DELETE FROM `pre_ucenter_failedlogins`";
        $res = mysqli_query($t->db,$sql);
        $rows2= mysqli_affected_rows($t->db);
        $t->close_db();
        if(-1!=$rows1 && -1!=$rows2)
        {
            show_msg('清除登录限制成功，用户可以登录了！', 'tools.php?action='.$action, 2000);
        }
        else
        {
            show_msg('清除登录限制失败，请联系管理员！', 'tools.php?action='.$action, 2000);
        }
    }
    else {
		show_header();
		...
		<table id="setadmin">
			...
			<input type="submit" name="setadminsubmit" value="重 &nbsp; 置">
		</form>
END;
		print<<<END
		<br/>
		...
		重置管理员<br/>
		<ul>
		<li>选择用户名或者UID(位于用户名后中括号内)。</li>
		<li>输入用户名或者UID(位于用户名后中括号内)。</li>
		<li>如果需要重置密码，输入密码。</li>
...
END;
		...
		<form action="?action={$action}" method="post">
		<h5>清除所有登录失败时间限制</h5>
			<input type="submit" name="clrtimelmtsubmit" value="清 &nbsp; 除">
		</form>
END;
		show_footer();
	}
    ...
    $result = mysqli_query($t->db,$sql);
    $is_available = mysqli_fetch_row($result)[0];
    ...
    $result = mysqli_query($t->db,$sql);
	$bbclosed = mysqli_fetch_row($result)[0];
	...
	$closedreason = mysqli_fetch_row(mysqli_query($t->db,$sql))[0];
                ...
                foreach($val AS $k => $v){
                    if(isset($v['f'])){
                        if($v['f'] < 1){
                            $k = intval($k);
                            // 删除db记录.
                            $sql = "SELECT * FROM  ".DB::table('forum_attachment')." WHERE aid = '$k'";
...
END;
		...
		while($list = mysqli_fetch_array($tablelist, MYSQL_ASSOC)) {
			if($type == 'allcheck' || $type == 'allrepair') {
				if($list['Engine'] != 'MEMORY' && $list['Engine'] != 'HEAP') {
					...
					$checkresult = mysqli_fetch_array($query, MYSQL_ASSOC);
		...
		if($type == 'allrepair') {
			foreach($tablelists as $table => $value) {
				...
				$query = mysqli_query($t->db,$sql);
				$repairresult = mysqli_fetch_array($query, MYSQL_ASSOC);
...
END;
    ...
    echo '<select class="cat_sel" name="pid">';
    echo $list = forumselect();
    echo '</select>';
            
    echo ' 今日主题数: <input style="width:60px" name="thcount" /> 今日帖题: <input style="width:60px" name="tothcount" />
    版块总帖数: <input style="width:60px" name="postscount" /> 
    昨天帖数: <input style="width:60px" name="yesterdayposts" />
     <input type="submit" value=" 修 改 " />';
    
    echo '<div><br>* 昨天帖数 要等于明天才会显示</div>';
	...
	if(!$_GET['importbak'] && !$_GET['nextfile']) {
	    ...
	    $fpush = $_GET['fpush']+0;
   		...
   		//检测是否关闭站点
		$sql = "SELECT svalue FROM ".DB_PRE."common_setting WHERE skey='bbclosed'";
		$closed = mysqli_fetch_row(mysqli_query($t->db,$sql))[0];
		if(!$fpush && !$closed) {
...
<script type="text/javascript">
    ...
    show_msg('退出成功！', 'tools.php?action=index', 2000);
...
function login_page() {
	...
	print<<<END
		...
		<form action="tools.php?action=login" method="post">
            ...
            <td colspan="2" style="color: #FF8040;">* tools工具箱 用于站点紧急维护 适用于 3.X 系列 [{$charset}]</td>
            <tr>
			</table>
...
function show_header() {
	...
	<title>Discuz! X3+ 急诊箱 For PHP7.x</title>
	<style type="text/css">
	...
	* {font-size:12px; color: #575757; font-family: Verdana, Arial, Helvetica, sans-serif; line-height: 1.5em; word-break: break-all; }
	...
	<body>
	<div class="bodydiv">
	<h1>Discuz! X3+ 急诊箱 For PHP7.x</h1><br/>
...
function show_footer() {
	...
	<div id="footer">Powered by {$tools_versions} &copy; Tencent Cloud Inc. 2001-2021 <a href="https://gitee.com/Discuz/DiscuzX" target="_blank">DiscuzX</a></div>
	</div>
	<br>
	</body>
	</html>
...
class T{
	...
	public function connect_db($type = 'db') {
		...
		if($dbhost && $dbuser && $dbpw && !$this->db = mysqli_connect($dbhost, $dbuser, $dbpw))
            show_msg('请检查config_global.php中数据库相关信息是否正确，与数据库服务器网络连接是否正常');

        if (0 != mysqli_connect_errno())
        {
            show_msg( "连接MySQL失败: " . mysqli_connect_error());
        }
	...
	private function _init_config() {
		...
		if(!@file_exists($uc_config_file)) {
			$error = true;
			$errormsg = '没有找到'.$uc_config_file.'文件，请检查 config_ucenter.php 是否存在或有读权限！';
		}
	...
	private function _init_db() {
		...
		$waiting = false;
		$waiting_msg = '';
		while($l = mysqli_fetch_array($query, MYSQL_ASSOC)) {
